...
# Reduce Kourier replicas
sed -i 's/\(.*replicas: \).*/\11/' test/config/ytt/ingress/kourier/kourier-replicas.yaml

# Build/push required ppc64le Serving test images
echo "Building required ppc64le Serving test images"
export KO_DOCKER_REPO="icr.io/upstream-k8s-registry/knative"
export KO_FLAGS="--platform=linux/ppc64le"
unset GOOS
unset GOARCH

ko publish --tags latest ${KO_FLAGS} ./test/test_images/config-mimic
ko publish --tags latest ${KO_FLAGS} ./test/test_images/grpc-ping
...
kubectl rollout status deployment/webhook -n knative-serving --timeout=300s || true
...
kubectl rollout status deployment/3scale-kourier-gateway -n kourier-system --timeout=300s || true
...
(
  ...
  while true; do
    kubectl get svc kourier -n kourier-system >/dev/null 2>&1 || {
      sleep 5
      continue
    }
    ...
    echo "$(date) port-forward exited, restarting..." >> /tmp/kourier-pf.log
    sleep 2
  done
) &

echo "$!" > /tmp/kourier-portforward.pid
EOF
