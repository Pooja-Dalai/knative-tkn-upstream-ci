...
# Stable Kourier Service port-forward
...
# Verify that the port-forward remains stable before tests begin.
...
# Run post-install script after ingress variables are configured.

POST_INSTALL_COMMAND='echo ">>> Running post-install fixes..." ; /tmp/post-install-fix.sh'

if ! grep -Fq '/tmp/post-install-fix.sh' 
test/e2e-common.sh; then
sed -i 
...
# Add cleanup before test failure and success.

if ! grep -Fq 'source /tmp/kourier-cleanup.sh' 
test/e2e-tests.sh; then
sed -i 
